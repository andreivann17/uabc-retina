  ...
  const filtered = useMemo(() => {
    ...
    return normalized
      ...
      .filter((r) => {
        ...
        if (activeFilter === "proliferative") return r.primaryStageKey === "proliferative";
        return true;
      });
            ...
            <div className="rr-headerStack">
              ...
              <div className="rr-pills">
                ...
                >
                 ...
                 <button
                  className={`rr-pill ${activeFilter === "proliferative" ? "rr-pillActive" : ""}`}
                  type="button"
                  onClick={() => setActiveFilter("proliferative")}
                >
                  Proliferative
                </button>
              </div>
                          ...
                          {m.diseases.map((d) => (
                            <div className="rr-miniRow" key={d.id_detection_disease}>
                              ...
                              <span className="rr-miniText">
                                {d.label} · summary {Number(d.summary)}
                              </span>
                                ...
                                <div className="rr-miniList" key={`stages-${d.id_detection_disease}`}>
                                  {d.stages.map((s) => (
                                    <div className="rr-miniRow" key={s.id_detection_stage}>
                                      ...
                                      <span className="rr-miniText">
                                        {s.label} · summary {Number(s.summary)}
                                      </span>
                                ...
                                <div className="rr-miniList" key={`bio-${d.id_detection_disease}`}>
                                  {d.biomarkers.map((b) => (
                                    <div className="rr-miniRow" key={b.id_detection_biomarker}>
                                      ...
                                      <span className="rr-miniText">
                                        {b.label} · summary {Number(b.summary)}
                                      </span>
...
function regroupModels(rawModels) {
  // 1) crea un map id_detection_model -> model (clonado)
  const map = new Map();

  for (const m of rawModels) {
    const key = Number(m?.id_detection_model);
    if (!key) continue;

    map.set(key, {
      ...m,
      diseases: [], // aquí vamos a reconstruir
    });
  }

  // helper: asegura que exista el model aunque no viniera (por si acaso)
  const ensureModel = (id_detection_model) => {
    const key = Number(id_detection_model);
    if (!key) return null;

    if (!map.has(key)) {
      map.set(key, {
        id_detection_model: key,
        id_detection: rawModels?.[0]?.id_detection,
        time_inference: "0",
        id_task: null,
        id_model: null,
        diseases: [],
  ...
  // helper: asegura disease dentro del model target
  const ensureDisease = (targetModel, baseDisease) => {
    const idDisease = Number(baseDisease?.id_detection_disease);
    if (!idDisease) return null;

    let d = targetModel.diseases.find((x) => Number(x?.id_detection_disease) === idDisease);
    if (!d) {
      d = {
        id_detection_disease: baseDisease.id_detection_disease,
        score: baseDisease.score,
        label: baseDisease.label,
        summary: baseDisease.summary,
        id_detection_model: targetModel.id_detection_model,
        stages: [],
        biomarkers: [],
      };
      targetModel.diseases.push(d);
    }
    return d;
  };

  // 2) recorre diseases base y distribuye stages/biomarkers a su model real
  for (const m of rawModels) {
    if (!Array.isArray(m?.diseases)) continue;

    for (const d of m.diseases) {
      // Disease pertenece al model indicado por d.id_detection_model (normalmente 312)
      const diseaseModel = ensureModel(d?.id_detection_model) || ensureModel(m?.id_detection_model);
      if (diseaseModel) {
        const baseDisease = { ...d, stages: [], biomarkers: [] };
        // OJO: guardamos disease sin stages/biomarkers aquí; los repartimos abajo
        ensureDisease(diseaseModel, baseDisease);
      }

      // stages: cada stage dice a qué model pertenece (en tu JSON: 313)
      if (Array.isArray(d?.stages)) {
        for (const s of d.stages) {
          const targetModel = ensureModel(s?.id_detection_model);
          if (!targetModel) continue;

          const diseaseInTarget = ensureDisease(targetModel, d);
          if (!diseaseInTarget) continue;

          diseaseInTarget.stages.push({
            ...s,
            id_detection_model: targetModel.id_detection_model,
          });
        }
      ...
      // biomarkers: cada biomarker dice a qué model pertenece (en tu JSON: 314)
      if (Array.isArray(d?.biomarkers)) {
        for (const b of d.biomarkers) {
          const targetModel = ensureModel(b?.id_detection_model);
          if (!targetModel) continue;

          const diseaseInTarget = ensureDisease(targetModel, d);
          if (!diseaseInTarget) continue;

          diseaseInTarget.biomarkers.push({
            ...b,
            id_detection_model: targetModel.id_detection_model,
          });
        }
  ...
  // 3) devuelve en el mismo orden original
  return rawModels
    .map((m) => map.get(Number(m?.id_detection_model)))
    .filter(Boolean);
}
...
function normalizeDetection(det) {
  ...
  const rawModels = Array.isArray(det?.models) ? det.models : [];
const models = regroupModels(rawModels);
...
function pickPrimaryStage(stages) {
  ...
  if (idx === 3) return { stageKey: "proliferative", stageLabel: "Proliferative" };
